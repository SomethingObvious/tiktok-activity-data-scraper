...
def parse_post(response: Response, favorite_video_ids: List[str]) -> Dict:
    ...
    log.debug(f"Content-Encoding: {encoding}")
    ...
    log.debug(f"Response content: {response_text[:500]}")
...
def load_urls_and_favorites_from_json(file_path: str, limit: int) -> (List[str], List[str]): # type: ignore
    with open(file_path, 'r', encoding='utf-8') as file:
        data = json.load(file)
    urls = [item['Link'] for item in data['Activity']['Like List']['ItemFavoriteList'][:limit]]
    urls = [re.sub(r'v(?!i)', '', url).replace('share', '@') for url in urls]
    favorite_video_ids = [item['Link'] for item in data['Activity']['Favorite Videos']['FavoriteVideoList']]
    favorite_video_ids = [re.sub(r'\D', '', url) for url in favorite_video_ids]
